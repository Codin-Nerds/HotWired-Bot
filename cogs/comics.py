...
class Comics(Cog):
    ...
    async def cah(self, ctx: Context) -> None:
        ...
        async with ctx.typing():
            ...
            embed = Embed(
                title="Random Cyanide and Happiness",
                color=Color.blurple()
            )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as r:
                if r.status == 200:
                    ...
                    embed = Embed(title=data["title"], description=data["alt"], color=Color.blurple())
